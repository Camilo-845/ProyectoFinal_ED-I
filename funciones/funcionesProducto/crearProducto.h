...
string incrementarCodigo (const string& codigo) {
    ...
    // Convertir el n�mero de vuelta a una cadena con relleno de ceros
    stringstream nuevoNumeroStr;
    nuevoNumeroStr << setw (4) << setfill ('0') << numero;

    // Combinar el prefijo con el nuevo n�mero
...
void crearProducto (Producto **cabezaPtr){
	...
	do {
		...
		cout << "           Ingrese el n�mero de art�culos disponibles" << endl;
		do {
			cout << "            (Debe ser una cantidad mayor a '0'): ";
			cin >> nuevo->stock;
			cout << endl;
		...
		cout << "     |  Ingrese (-1) para salir del men�, si desea agregar un nuevo producto ingrese (1): "; 
		cin >> respuesta;
		cout << "      -------------------------------------------------------------------------------------" << endl;
		system ("pause");
		system ("CLS");
